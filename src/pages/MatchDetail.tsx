  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={`${match.statusText} - ${match.tournament} at ${match.venue}`} />
      </Helmet>
      ...
      <section className="bg-primary text-primary-foreground py-8">
        <div className="container-content">
          ...
          <div className="text-center bg-primary-foreground/10 rounded-lg py-3 px-4">
            <p className="font-medium">{match.statusText}</p>
            {match.context && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-foreground font-medium">
                    {match.currentBowler.wickets}/{match.currentBowler.runs} ({match.currentBowler.overs}) • Econ: {match.currentBowler.economy.toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {match.innings.map((inn, idx) => (
              <div key={idx} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                <span className="text-foreground font-medium">{inn.team.shortName}</span>
                <span className="text-foreground">
                  {inn.runs}/{inn.wickets} ({inn.overs} ov) • RR: {inn.runRate.toFixed(2)}
                </span>
              </div>
            ))}
          </div>
